...
"""
可除颤心律信号识别
双核卷积+顺序池化+LSTM网络(output层combine)
最新修改于 2019年7月16日
"""
import warnings
...
units = 60
path = 'CNN_SP_LSTM\\double_stream\\units ='+str(units)
...
for k in range(len(cv)):   #交叉验证
    ...
    for i in range(2):
        ...
        locals()['tensor'+str(i)] = LSTM(units = units,name='lstm_'+str(i)) (locals()['tensor'+str(i)])
    ...
    # 保存模型
    model.save('.\model\\'+path+'\\'+str(signal_len)+'s-train_'+str(k+1)+'-fold.h5py')
...
print('双核CNN-SP-LSTM units = %d'%units)
