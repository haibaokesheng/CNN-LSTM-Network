# -*- coding: utf-8 -*-
"""
特征：斜率顺序统计量特征 slope order statistics
分类器：BP神经网络分类,卷积权重[1,-1,0,0]
2019年3月22日
最新修改于2019年6月20日
"""
import warnings
...
path = 'BP_sos\\BP_sos_units='+str(units_num)
...
for signal_len in length_list[:1]:
    ...
    for k in range(len(cv)):  
        ...
        # 构建卷积-顺序池化神经网络,‘channels_last’模式下，输入形如（samples,rows,cols,channels）的4D张量
        input0 = Input(shape=(X_train.shape[1],X_train.shape[2],1),name='input0')
        conv2d = Conv2D(filters=1,kernel_size=[1,4],strides=1, padding='valid',use_bias=0,name='conv2d',trainable=False)
...
print('斜率顺序统计量特征+BP,units=%d'%(units_num))
