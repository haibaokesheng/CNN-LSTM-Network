...
data_mved_123  = sio.loadmat(r'.\ECG_data\MVED_data\MVED_123\ECG_MVED_'+str(signal_len)+'s_123.mat')['data_ecg_MVED_123']
## 加载 13-17 Hz的数据  VMED 的67
data_mved_67  = sio.loadmat(r'.\ECG_data\MVED_data\MVED_67\ECG_MVED_'+str(signal_len)+'s_67.mat')['data_ecg_MVED_67']
...
for k in range(len(cv)):   #交叉验证
   ...
   # a = adam(lr=0.005)
    ...
    history = model.fit(X_train,Y_train_oh,batch_size=256,epochs=40,validation_data=(X_test,Y_test_oh))
    # 保存模型
    ...
    important_data = {'train_loss':train_loss,'test_loss':test_loss,
                      ...
                      #'threshold':thr,
                      'pred_trainY':pred_trainY,
                      'pred_testY':pred_testY,
